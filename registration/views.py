...
@login_required
def export_fees_view(request):
    """
    GET (no session_id)  → render filter form
    GET (?session_id=…)  → stream CSV download
    """
    sessions = AcademicSession.objects.all().order_by('-start_date')
    active_session = AcademicSession.get_active()
    ...
    if not session_id:
        return render(request, 'registration/export_fees.html', {
            'sessions': sessions,
            'active_session': active_session,
    ...
    # Pre-fetch FeeDetails and Installments in bulk to avoid N+1
    ...
    CSV_HEADERS = [
        # A – Student
        'student_id', 'student_name', 'phone', 'email', 'gender', 'dob',
        'date_of_joining', 'school',
        # B – Parent
        'father_name', 'mother_name', 'father_contact', 'mother_contact',
        # C – Enrollment
        'session', 'class', 'course_offering', 'subjects',
        # D – Fee Summary
        'course_annual_fee',
        'tuition_fees',
        'subjects_fee_detail',
    ...
    def _subjects_fee_detail(course_offering, enrolled_subject_ids, annual_fee):
        """
        Returns a semicolon-separated string of per-subject tuition breakdown.
        ...
        for cos in course_offering.course_subjects.all():
            if cos.subject_id in enrolled_subject_ids:
                ...
                parts.append(f"{cos.subject.name}[{cos.percentage}%]={fee}")
        return '; '.join(parts)
    ...
    def generate_rows():
        ...
        for enrollment in qs:
            student = enrollment.student
            user = student.user
            parent = getattr(student, 'parent_details', None)
            ...
            subjects_str = ', '.join(
                s.name for s in sorted(enrollment.subjects.all(), key=lambda s: s.name)
            )
            ...
            if annual_fee is not None:
                book_fees_derived = (Decimal(str(annual_fee)) * Decimal('0.15')).quantize(Decimal('1'), rounding=ROUND_HALF_UP)

            subjects_fee_detail = _subjects_fee_detail(co, enrolled_subject_ids, annual_fee)

            # Section A+B+C+D (shared across installment rows)
            base_row = [
                str(student.stu_id),
                ...
                student.doj.strftime('%Y-%m-%d') if student.doj else '',
                student.school_name or '',
                # parents
                (parent.father_name or '') if parent else '',
                (parent.mother_name or '') if parent else '',
                (parent.father_contact or '') if parent else '',
                (parent.mother_contact or '') if parent else '',
                # enrollment
                str(enrollment.session),
                str(enrollment.class_name),
                str(co) if co else '',
                subjects_str,
                # fee summary
                str(annual_fee) if annual_fee is not None else '',
                str(fd.tuition_fees) if fd else '',
                subjects_fee_detail,
                str(book_fees_derived),
                str(fd.cab_fees) if fd else '',
                str(fd.discount) if fd else '',
                str(fd.registration_fee) if fd else '',
                'Yes' if (fd and fd.book_discount) else 'No',
                'Yes' if (fd and fd.registration_discount) else 'No',
                'Yes' if (fd and fd.gst_opted_in) else 'No',
                str(fd.tuition_gst) if fd else '',
                str(fd.book_gst) if fd else '',
                str(fd.transport_gst) if fd else '',
                str(fd.total_fees) if fd else '',
                str(fd.paid_amount) if fd else '',
                str(fd.remaining_balance) if fd else '',
            ]

            if not installments:
                # One row with blank installment section
                yield base_row + ['', '', '', '', '', '']
            else:
                for ins in installments:
                    yield base_row + [
                        ins.label or '',
                        ins.due_date.strftime('%Y-%m-%d') if ins.due_date else '',
                        str(ins.amount),
                        ins.payment_type or '',
                        'Yes' if ins.paid else 'No',
                        ins.remark or '',
                    ]
    ...
    filename = f"fees_{session_slug}_{today}.csv"
